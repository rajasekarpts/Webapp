'------------------------------------------------------------------------------
' <auto-generated>
'     This code was generated from a template.
'
'     Manual changes to this file may cause unexpected behavior in your application.
'     Manual changes to this file will be overwritten if the code is regenerated.
' </auto-generated>
'------------------------------------------------------------------------------

Imports System
Imports System.Data.Entity
Imports System.Data.Entity.Infrastructure

Partial Public Class inventoryEntities
    Inherits DbContext

    Public Sub New()
        MyBase.New("name=inventoryEntities")
    End Sub

    Protected Overrides Sub OnModelCreating(modelBuilder As DbModelBuilder)
        Throw New UnintentionalCodeFirstException()
    End Sub

    Public Overridable Property C__EFMigrationsHistory() As DbSet(Of C__EFMigrationsHistory)
    Public Overridable Property AspNetRoleClaims() As DbSet(Of AspNetRoleClaim)
    Public Overridable Property AspNetRoles() As DbSet(Of AspNetRole)
    Public Overridable Property AspNetUserClaims() As DbSet(Of AspNetUserClaim)
    Public Overridable Property AspNetUserLogins() As DbSet(Of AspNetUserLogin)
    Public Overridable Property AspNetUsers() As DbSet(Of AspNetUser)
    Public Overridable Property AspNetUserTokens() As DbSet(Of AspNetUserToken)
    Public Overridable Property BookGalleries() As DbSet(Of BookGallery)
    Public Overridable Property Books() As DbSet(Of Book)
    Public Overridable Property Languages() As DbSet(Of Language)

End Class
